...
//    }
    ...
    public void checkUniverse2(){
        ExistingCreatures.addCreature();
        for(ExistingCreatures sample: ExistingCreatures.ExistentCreatures) {
            if ((isHuman == null || isHuman == sample.getIsHuman())
                    && (planet == null || planet.equals(sample.getPlanet()))
                    && (age == null || (age >= sample.getAge1() && age <= sample.getAge2()))
                    && (traits == null || containsAnyTrait(traits, sample.getTraits())))
            {
                universe = sample.getUniverse(); break;
            }
        }
    }
